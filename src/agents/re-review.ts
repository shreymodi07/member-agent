...
import { ReviewStorage } from '../utils/review-storage';
import { StoredReview, StoredIssue, ReReviewResult } from '../types/review-storage';
...
export interface ReReviewOptions {
  projectPath: string;
  reviewId?: string; // If not provided, uses latest
}

export class ReReviewAgent {
  private aiProvider: AIProvider;
  private codeReviewAgent: CodeReviewAgent;

  constructor(config?: AgentConfig) {
    this.aiProvider = new AIProvider(config);
    this.codeReviewAgent = new CodeReviewAgent(config);
  }

  async reReview(options: ReReviewOptions): Promise<ReReviewResult> {
    const storage = new ReviewStorage(options.projectPath);
    ...
    const currentCodeMap = new Map<string, string>();
    const previousCodeMap = new Map<string, string>();
    ...
    const fixedIssues: StoredIssue[] = [];
    const unfixedIssues: StoredIssue[] = [];
    ...
    const newIssues = this.filterNewIssues(
      newReviewResult.issues.map((issue, idx) => ({
        id: `N${idx + 1}`,
    ...
    // Generate summary
    const summary = await this.generateReReviewSummary(
      previousReview,
      fixedIssues,
      unfixedIssues,
      newIssues
  ...
  private async verifyIssueFixed(
    issue: StoredIssue, 
    currentCode: string, 
    previousCode?: string
  ): Promise<boolean> {
...
Response:`;
  ...
  private getCurrentGitCommit(projectPath: string): string | null {
    try {
  ...
  private getFileAtCommit(projectPath: string, filePath: string, commit: string): string | null {
    try {
  ...
  private hasCodeChangedAroundLine(
    ...
    const endLine = Math.min(
      Math.max(prevLines.length, currLines.length),
  ...
  private filterNewIssues(
    currentIssues: StoredIssue[],
    previousIssues: StoredIssue[]
  ): StoredIssue[] {
  ...
  private async generateReReviewSummary(
